...
const createMenu = (itens) => {
  const objectEntries = Object.entries(itens);
  ...
  const menu = {
    ...
    consumption,
    order: (newItem) => {
      consumption.push(newItem);
    },
    pay: () => {
      let total = 0;
      consumption.forEach((element) => {
        objectEntries.forEach((elementEntries) => {
          const arrayOfKeysOfEachType = Object.keys(elementEntries[1]);
          const arrayOfValuesOfEachType = Object.values(elementEntries[1]);
          arrayOfKeysOfEachType.forEach((e, indexInKeys) => {
            if (e === element) {
              total += parseFloat(arrayOfValuesOfEachType[indexInKeys]);
            }
          });
        });
      });
      return Number((1.1 * total).toFixed(2));
    },
  };
