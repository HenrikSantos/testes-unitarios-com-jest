...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  it('Verifica se ao receber um raio, a função `circle` retorna um objeto contendo os valores esperados', () => {
    ...
    expect(Object.values(circle(2))).toHaveLength(3);
    ...
    expect(Object.entries(circle(3))[1][1]).toBeCloseTo(28.26);
    // Teste se a função retorna, em um objeto, os dados corretos de um círculo de raio 3.
    const circuloDeRaio3 = Object.values(circle(3))
    expect(typeof circle(3)).toBe('object')
    expect(circuloDeRaio3[0]).toBe(3);
    expect(circuloDeRaio3[1]).toBeCloseTo(28.26);
    expect(circuloDeRaio3[2]).toBe(18.84);
  });
});
